...
def main() -> int:
    ...
    command = ["./collect.sh", "--pid", str(a.pid), "--duration", str(a.duration), "--out", a.out]
    if a.unit:
        command += ["--systemd-unit", a.unit, "--coredump"]
    print("# Run as the service user from the verified collector kit directory")
    print(" ".join(shlex.quote(x) for x in ["./collect.sh", "--check", "--pid", str(a.pid)]))
    print(" ".join(shlex.quote(x) for x in command + ["--dry-run"]))
    print("# After reviewing the check and dry-run output")
    print(" ".join(shlex.quote(x) for x in command + ["--yes"]))
